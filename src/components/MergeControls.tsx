    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        {isMerging ? (
          <button
            onClick={onAbort}
            className="flex items-center gap-2 rounded-lg border border-red-200 bg-red-50 px-5 py-3 text-sm font-medium text-red-600 hover:bg-red-100 transition-colors active:bg-red-200 min-h-[44px]"
          >
          ...
          <button
            ...
            disabled={!canMerge}
            className={[
              'flex items-center gap-2 rounded-lg px-6 py-3 text-sm font-semibold transition-all min-h-[44px]',
              canMerge
        ...
        {status === 'completed' && telemetry.merge_duration_ms != null && (
          <p className="text-xs text-slate-400 tabular-nums">
            완료 · {telemetry.merge_duration_ms}ms · {(telemetry.total_rows ?? 0).toLocaleString()}행
