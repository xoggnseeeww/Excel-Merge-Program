...
export default function HomePage() {
  ...
  useEffect(() => {
    if (status === 're_validating') handleReValidate();
  }, [status, handleReValidate]);

  const isProcessing = ['validating', 'parsing', 're_validating', 'merging'].includes(status);

  return (
    <main className="mx-auto w-full max-w-2xl px-4 sm:px-6 py-6 sm:py-12">
      {/* 헤더 */}
      <div className="mb-8 sm:mb-10 text-center">
        ...
        <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-slate-900">Excel Merger</h1>
      ...
      <div className="space-y-5 sm:space-y-6">
        ...
        {status === 'completed' && (
          <button
            onClick={reset}
            className="w-full rounded-lg border border-slate-200 py-3 text-sm text-slate-500 hover:bg-slate-50 transition-colors active:bg-slate-100"
          >
            새로 시작하기
          </button>
        )}
      ...
      {/* 푸터 */}
      <footer className="mt-12 sm:mt-16 text-center text-xs text-slate-300 space-y-2">
        <p>파일은 브라우저 메모리에서만 처리됩니다 · 서버로 전송되지 않습니다</p>
        <p className="hidden sm:block">Chrome · Firefox · Safari · Edge 최신 버전 지원 · IE 미지원</p>
